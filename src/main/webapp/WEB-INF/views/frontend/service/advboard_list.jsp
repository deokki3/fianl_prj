							...
							<div class="row">
								...
								<select class="form-control col-sm-2 p-2" name="field">
									<option value="nickname"
										<c:if test="${field=='nickname'}">selected</c:if>>작성자</option>
									<option value="adv_title"
										<c:if test="${field=='adv_title'}">selected</c:if>>제목</option>
									<option value="adv_content"
										<c:if test="${field=='adv_content'}">selected</c:if>>내용</option>
									<option value="combined"
										<c:if test="${field=='combined'}">selected</c:if>>제목+내용</option>
								...
								<div class="ml-md-auto" style="margin-right: 20px;">
									<button type="button" id="insertBtn"
										...
										onclick="clickInsert()">
										문의하기 <i class="icon-arrow-right-circle"></i>
									</button>
								</div>
							</div>
						</form>
